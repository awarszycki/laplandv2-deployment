...
export default function Finanse({ members, expenses, currentUser, onAddExpense, onUpdateExpense, onDeleteExpense }) {
  ...
  return (
    <div className="finanse-container">
                ...
                <li key={e.id} className="expense-item">
                  ...
                  <div className="expense-end-block">
                    ...
                    <div className="expense-actions-row">
                      ...
                      {/* ✨ ZMIENIONO: onClick otwiera dialog zamiast usuwać bezpośrednio */}
                      <button 
                        className="btn-icon-action btn-delete-icon" 
                        onClick={() => setDeleteConfirm({ id: e.id, title: e.title })}
                        title="Usuń"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* ✨ NOWY DIALOG dla potwierdzenia usuwania wydatku */}
      <ConfirmDialog
        isOpen={deleteConfirm !== null}
        title="Usunąć wydatek?"
        message={`Czy na pewno chcesz usunąć wydatek "${deleteConfirm?.title}"? Operacja jest nieodwracalna.`}
        confirmText="Usuń"
        cancelText="Anuluj"
        isDangerous={true}
